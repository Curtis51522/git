...
def _create_production_batches(items):
    ...
    for item in items:
        ...
        freshness = str(item.get("freshness", "Fresh")).strip()
        if freshness not in allowed_freshness:
            raise HTTPException(
                400,
                f"Invalid freshness for '{product_name}': expected Fresh or Day-1",
            )
    ...
    try:
        ...
        for item in items:
            ...
            freshness = str(item.get("freshness", "Fresh")).strip()
            production_time = now - timedelta(days=1) if freshness == "Day-1" else now
            tray_color = (
                "yellow"
                if freshness == "Day-1"
                else item.get("tray_color", "green")
            )
            sales_area = "Day-1 Area" if freshness == "Day-1" else "Fresh Area"
            ...
            q(db, "batch_inventory").insert({
                ...
                "production_time": production_time.isoformat(),
