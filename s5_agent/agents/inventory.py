...
class InventoryAgent:
    ...
    def analyze_for_graph(self, raw: Dict[str, Any], params: Dict[str, Any]) -> AgentOutput:
        ...
        flow_opening_units = int(data.get("flow_opening_units", 0) or 0)
        flow_baked_units = int(data.get("flow_baked_units", 0) or 0)
        flow_available_units = int(data.get("flow_available_units", 0) or 0)
        ...
        return AgentOutput(
            ...
            metrics={
                ...
                "flow_opening_units": flow_opening_units,
                "flow_baked_units": flow_baked_units,
                "flow_available_units": flow_available_units,
            ...
            evidence_items=[
                ...
                EvidenceItem(
                    id="flow_opening_units",
                    source="inventory_flow",
                    description="Finished-product units available at the start of the selected date",
                    value=flow_opening_units,
                ...
                EvidenceItem(
                    id="flow_available_units",
                    source="inventory_flow",
                    description="Opening finished stock plus units baked on the selected date",
                    value=flow_available_units,
                    metadata={"record_count": flow_record_count},
                ),
                EvidenceItem(
                    id="flow_sell_through_pct",
                    source="inventory_flow",
                    description="Sold units as a share of opening stock plus units baked on the selected date",
