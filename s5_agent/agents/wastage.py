...
class WastageAgent(BaseAgent):
    ...
    def analyze(self, raw, params, context="", history="", key_metrics=None):
        ...
        if anomalies:
            ...
            thr = THRESHOLDS["wastage_abnormal_rate"] * 100
            return AgentOpinion(agent=self.name, opinion=f"Abnormal wastage (> {thr:.0f}% threshold): " + ", ".join(names),
        ...
        thr = THRESHOLDS["wastage_abnormal_rate"] * 100
        return AgentOpinion(agent=self.name, opinion=f"Wastage normal (all rates < {thr:.0f}% threshold)", confidence=0.8,
