...
class ProductionPlanAgent(BaseAgent):
    ...
    def analyze(self, raw, params, context="", history="", key_metrics=None):
        ...
        if buffer and buffer < 1.0:
            recommendations.append({
                "action": "Increase buffer to at least 110% to cover forecast uncertainty",
                "urgency": "high", "time_horizon": "tomorrow",
                "rationale": "Buffer below 100% risks stockouts if actual demand exceeds forecast",
...
def _query_plan(date_str="", params=None):
    try:
        ...
        url = api_url("s3/plan/7day") + "?" + urlencode({"date": start_date})
