...
def _synthesize_inventory_summary(outputs: dict[str, Any]) -> str:
    ...
    if product_count == 0:
        ...
        if snapshot_basis == "selected_date_dashboard" and snapshot_date:
            opening = (
                f"For {snapshot_date}, finished-product stock totals {total_units} units across {product_count} tracked products, "
                f"averaging {units_per_product:.2f} units per product."
            )
        else:
            opening = (
                f"Current finished-product stock totals {total_units} units across {product_count} tracked products, "
                f"averaging {units_per_product:.2f} units per product."
            )
        if thin_count:
            opening += (
        ...
        operations = [
            f"The snapshot contains {fresh_units} fresh units and {day1_units} day-1 units."
        ]
        if day1_count:
            operations.append(
    ...
    if flow_record_count:
        ...
        movement = (
            ...
            f"{'es' if flow_record_count != 1 else ''}: {flow_baked_units} units were baked, "
            f"{flow_sold_units} were sold, {flow_discarded_units} "
            f"{'were' if flow_discarded_units != 1 else 'was'} discarded, and {flow_left_units} remain in those batches. "
