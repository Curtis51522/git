    ...
    for d in range(days):
        ...
        for prod in PRODUCT_TYPES:
            ...
            rows.append({
                ...
                "day_of_week": dow, "is_weekend": we,
                "day_of_month": dt.day, "month": mth,
...
def main():
    ...
    print(f"       {len(df):,} rows | {n_dates} dates | {n_products} products")
    ...
    def save_error_stats(y_true, y_pred, df_val, product_name):
        r = (y_true - y_pred).values
        errors = {"std_dev": float(np.std(r)), "p5": float(np.percentile(r,5)), "p95": float(np.percentile(r,95)), "mae": float(np.mean(np.abs(r))), "n_samples": len(r)}
    ...
    for prod in PRODUCT_TYPES:
        ...
        # Freshness features removed - predicting total daily sales
